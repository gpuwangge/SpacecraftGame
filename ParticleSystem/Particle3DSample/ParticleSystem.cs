...
namespace Particle3D
{
    ...
    public abstract class ParticleSystem : DrawableGameComponent
    {
        ...
        public void AddParticle(Vector3 position, Vector3 velocity)
        {
            ...
            double xAngle = random.NextDouble() * MathHelper.TwoPi;
            double yAngle = random.NextDouble() * MathHelper.TwoPi;
            double zAngle = random.NextDouble() * MathHelper.TwoPi;

            float xVelocity = MathHelper.Lerp(settings.MinXVelocity,
                                                       settings.MaxXVelocity,
                                                       (float)random.NextDouble());
            float yVelocity = MathHelper.Lerp(settings.MinYVelocity,
                                           settings.MaxYVelocity,
                                           (float)random.NextDouble());
            float zVelocity = MathHelper.Lerp(settings.MinZVelocity,
                                           settings.MaxZVelocity,
                                           (float)random.NextDouble());
            velocity.X += xVelocity * (float)Math.Cos(xAngle);
            velocity.Y += yVelocity * (float)Math.Cos(yAngle);
            velocity.Z += zVelocity * (float)Math.Sin(zAngle);
